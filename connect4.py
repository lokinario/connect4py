...
def heuristic(board, player_piece):
    score = 0
    for row in board:
        score += windowScore(row, player_piece)
    return score


def abprune(board: np.ndarray, depth: int, alpha: int, beta: int, maximizingPlayer: int):
...
while 1:
    ...
    while not game_over:
        if turn == 0:
            ...
            while 1:
                move = int(
                    input("input 0-6 to choose what column to drop your piece in: "))
            ...
            col, score = abprune(board, 7, -math.inf, math.inf, 1)
